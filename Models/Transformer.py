...
class Transformer(nn.Module):
    def __init__(self, embed_dim=384, depth=14, num_heads=6, mlp_ratio=3.):
        ...
        self.encoderlayer_co = TransformerEncoder_co(embed_dim=embed_dim, depth=depth, num_heads=num_heads,mlp_ratio=mlp_ratio)
        self.encoderlayer = TransformerEncoder(embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio)
        self.co_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.mlp =   nn.Sequential(nn.Linear(embed_dim, embed_dim),nn.GELU())
        # self.mlp_rgb = nn.Sequential(nn.Linear(embed_dim, embed_dim), nn.GELU())

    def forward(self, rgb_fea, lat_fea):
    
        _, shape, _ = rgb_fea.shape
        co_tokens = self.co_token.expand(rgb_fea.shape[0], -1, -1)#1*1*384 all zero
        rgb_fea = torch.cat((co_tokens, rgb_fea, lat_fea), dim=1)#1*981*384

        rgb_memorys = self.encoderlayer_co(rgb_fea)
        co_token, rgb_memory, lat_memory =rgb_memorys[:, 0, :], rgb_memorys[:, 1:shape+1, :], rgb_memorys[:, shape+1:, :]

        return rgb_memory, lat_memory, self.mlp(co_token)
...
class token_Transformer(nn.Module):
    ...
    def forward(self, rgb_fea, lat_fea):
        B, _, _ = rgb_fea.shape
        fea_1_16 = torch.cat([rgb_fea, lat_fea], dim=2)
        fea_1_16 = self.mlp_s(self.norm(fea_1_16))   # [B, 14*14, 384]
        fea_1_16 = fea_1_16[:, :, :384]
