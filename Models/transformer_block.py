...
class MutualAttention(nn.Module):
    ...
    def forward(self, rgb_fea, depth_fea):
        ...
        depth_q = self.rgb_q(depth_fea).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        depth_k = self.rgb_k(depth_fea).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        depth_v = self.rgb_v(depth_fea).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        depth_fea = self.rgb_proj(depth_fea)
