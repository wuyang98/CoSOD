...
from tools import calc_precision_and_jaccard
import numpy as np
            ...
            for i in range(len(image_list)):
                ...
                if divided != 0:
                    for k in range(divided):
                        ...
                        group_gt = cur_class_gt[(k * group_size): ((k + 1) * group_size)]
                        group_gt = group_gt.unsqueeze(1).cuda()
                        outputs_saliency, _ = net(group_rgb, group_gt)
                        mask_1_16, mask_1_8, mask_1_4, mask_1_1 = outputs_saliency
                        # outputs, _ = net(group_rgb, group_gt)
                        # outputs_saliency, _ = outputs
                        # mask_1_16, mask_1_8, mask_1_4, mask_1_1 = outputs_saliency
                        output_s_1 = torch.sigmoid(mask_1_1)
                ...
                if rested != 0:
                    ...
                    mask_1_16, mask_1_8, mask_1_4, mask_1_1 = outputs_saliency
                    output_s_1 = torch.sigmoid(mask_1_1)
                    cur_class_mask[(divided * group_size):] = output_s_1[:rested].squeeze(1)
